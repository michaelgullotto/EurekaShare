...
public class LivekitSamples : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Testing background
        if (mode == "pub" && captureCamera != null)
        {
            captureCamera.backgroundColor = Random.ColorHSV();
        }
    }
    ...
    IEnumerator AutoBoot()
    {
        UpdateStatusText($"Booting {identity} ({mode})");

        // For local-only testing keep localhost:
        // url = "ws://127.0.0.1:7880";

        // 1) Connect
        yield return MakeCall(); // uses url + token; we¡¯ll replace token generation next
    ...
    { // Choose capture camera
      var cam = captureCamera != null ? captureCamera : Camera.main; 
        if (cam == null) 
        { 
            Debug.LogError("No capture camera found. Assign captureCamera in Inspector or ensure there is a MainCamera."); 
            yield break; 
        } 
        // Create video source from camera
        var source = new CameraVideoSource(cam); 
        // (Optional) show local preview tile
        GameObject imgObject = new GameObject($"local_{identity}"); 
        RectTransform trans = imgObject.AddComponent<RectTransform>(); 
        trans.localScale = Vector3.one; 
        trans.sizeDelta = new Vector2(180, 120); 
        RawImage image = imgObject.AddComponent<RawImage>(); 
        source.TextureReceived += (txt) => { if (image != null) image.texture = txt; }; 
        imgObject.transform.SetParent(layoutGroup.gameObject.transform, false); 
        // Create & publish track
        var track = LocalVideoTrack.CreateVideoTrack($"video_{identity}", source, room); 
        var options = new TrackPublishOptions(); 
        options.VideoCodec = VideoCodec.H264; // start with H264 for compatibility
        options.Simulcast = false; 
        options.Source = TrackSource.SourceCamera; 
        var videoEncoding = new VideoEncoding 
        { 
            MaxBitrate = 512000, // 0.5 Mbps (tune later)
            MaxFramerate = frameRate 
        }; 
        options.VideoEncoding = videoEncoding; 
        var publish = room.LocalParticipant.PublishTrack(track, options); 
        yield return publish; 
        if (!publish.IsError) 
            Debug.Log($"Track published! identity={identity}"); 
        else 
            Debug.LogError($"Publish error."); 
    ...
    private void OnDestroy() { if (webCamTexture != null) { webCamTexture.Stop(); } }
    #endregion
